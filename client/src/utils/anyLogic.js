class Utils {
    ...
    static _convertFromString(value, type) {
        ...
        return Utils._tryToParseJSON(value);
    }

    static _tryToParseJSON(value, fallback = null) {
        try {
...
class MultiRunOutputs {
    constructor(resultData) {
        ...
        this.inputsList = Utils._tryToParseJSON(inputsArray.value, []);

        this.outputColumns = resultData
            .filter(d => d.outputs.length > 0)
            .map(d => ({ name: d.outputs[0].name, value: Utils._tryToParseJSON(d.value) }));
    ...
    getValue(pathToField) {
        if (this.version.greaterOrEquals(this.cloudClient.VERSION_SUPPORTING_EXTENDED_ANIMATION_API)) {
            ...
            return this.cloudClient._apiRequest(url, "GET")
                .then(r => {
                    return Utils._tryToParseJSON(Utils._tryToParseJSON(r)); // Due to double serialization on server side
                });
        } else {
...
class ModelRun {
    ...
    getProgress() {
        if (this.runState) {
            return Promise.resolve(this.runState.message === "" ? undefined : Utils._tryToParseJSON(this.runState.message));
        } else {
            return this.client._apiRequest(this.versionsUrl + "/run", "POST", this._getRequestParams())
                .then(runState => {
                    return runState.message === "" ? undefined : Utils._tryToParseJSON(runState.message);
                });
        }
    }
...
class CloudClient {
    ...
    getFileHash(file) {
        return this._readUploadFile(file)
            .then((content) => this._getHashString(content));
    }
    ...
    async uploadFile(input) {
        const file = await this._readUploadFile(input.files[0]);
        const hash = this._getHashString(file);
        const existByHash = await this.fileExistsByHash(hash);
        const uploadUrl = this.OPEN_API_URL + "/upload-file/";
        return existByHash ?
            new Promise((resolve) => resolve(hash)) :
            this._apiRequest(uploadUrl, "POST", {data: file});
    }
                ...
                if (this.readyState === 4) {
                    if (this.status === 200) {
                        ...
                        if (!params.responseType)
                            result = Utils._tryToParseJSON(result);
                        resolve(result);
                    } else {
                        if (!params.responseType) {
                            reject(Utils._tryToParseJSON(this.responseText));
                        } else {
                            reject(this.status);
                        }
                    }
